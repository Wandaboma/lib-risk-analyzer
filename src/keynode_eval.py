...
def page_rank(graph,alpha=0.85,tol=1e-8): #tol为收敛域值
    ...
    normalized_downloads = (downloads - min_dl) / range_dl
    nstart = {crate_id: normalized_downloads[i] for i, crate_id in enumerate(crate_ids)}
    return nx.pagerank(graph, alpha=0.85, max_iter=len(nx.dag_longest_path(graph)) + 5,nstart=nstart)
...
def get_dependents_with_names(crate_name):
    """
    给定一个 crate 名称，返回所有直接依赖它的包（即 dependents）
    :param crate_name: 要查询的包名
    :return: list of (dependent_id, dependent_name)
    """

    target_id = None
    for crate_id, name in crate_id_to_name.items():
        if name == crate_name:
            target_id = crate_id
            break
    ...
    dependents = []
    for pred_id in graph.predecessors(target_id):
        pred_name = graph.nodes[pred_id].get("name", "(unknown)")
        dependents.append((pred_id, pred_name))
...
if __name__ == "__main__":
    ...
    # dependents = get_dependents_with_names(crate)
    # if dependents is None:
    #     print(f"Crate '{crate}' not found.")
    # elif not dependents:
    #     print(f"Crate '{crate}' has no dependents.")
    # else:
    #     print(f"{crate} is directly depended on by {len(dependents)} crates:")
